...
void Window1::window1_setup(bool* window_1){
/**
    ...
    ImGui::SetNextWindowPos(ImVec2(200, 19));
*/
    ...
    ImGui::SetNextWindowPos(ImVec2(main_window_dimentions->Size.x / 3, (yPos-19)/2 + 19));
}

void Window1::window1_body(bool* window_1, pqxx::result* result_for_table_tab1) {
    ImPlot::CreateContext();
    ImGui::Begin("Window 1", window_1, ImGuiWindowFlags_HorizontalScrollbar | //ImGuiWindowFlags_MenuBar
    ...
    Window1::window1_plots(result_for_table_tab1);

    ImGui::End();
    ImPlot::DestroyContext();
}

void Window1::window1_plots(pqxx::result* result_for_table_tab1) {
    ...
    if(!result_for_table_tab1->empty()){
        ...
        if(column_names.size() < result_for_table_tab1->columns()) {
            for (int i = 0; i < result_for_table_tab1->columns(); ++i){ //for every column //max 8
                std::vector<int> column_values;
                for (int j = 0; j < result_for_table_tab1->size(); ++j) { //for every row //max 14
                    column_values.push_back(vector2d[j][i]);
                }
                column_names.insert(std::make_pair(result_for_table_tab1->column_name(i), column_values));
            }
        }
                                            /**
                                            //print
        ...
        if(!column_names.empty()) {
            for (std::unordered_map<std::string, std::vector<int>>::const_iterator ite = column_names.begin(); ite != column_names.end(); ++ite) {
                ...
                for (auto each : column_names){
                    ...
                    float corr = Window1::calculate_correlation(&vector_one, &vector_two);
        ...
        float array2d_corr_data[result_for_table_tab1->columns()][result_for_table_tab1->columns()];
        if(!corr_data.empty()){
            for(int i = 0; i < result_for_table_tab1->columns(); ++i) {
                for(int j = 0; j < result_for_table_tab1->columns(); j++){
        ...
        const char* xlabels1[result_for_table_tab1->columns()];
        const char* ylabels1[result_for_table_tab1->columns()];
...
*/
        ...
        if (ImPlot::BeginPlot("##Heatmap1",ImVec2(425,425),ImPlotFlags_NoLegend|ImPlotFlags_NoMouseText)) {
            ...
            ImPlot::SetupAxisTicks(ImAxis_X1,0 + 1.0/14.0, 1 - 1.0/14.0, result_for_table_tab1->columns(), xlabels1);
            ImPlot::SetupAxisTicks(ImAxis_Y1,1 - 1.0/14.0, 0 + 1.0/14.0, result_for_table_tab1->columns(), ylabels1);
            ImPlot::PlotHeatmap("heat",array2d_corr_data[0],result_for_table_tab1->columns(),result_for_table_tab1->columns(),scale_min1,scale_max1,nullptr,ImPlotPoint(0,0),ImPlotPoint(1,1),hm_flags1);
            ImPlot::EndPlot();
        }
    }
...
float Window1::calculate_correlation(std::vector<int>* vector_one, std::vector<int>* vector_two){
